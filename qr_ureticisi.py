...
def showSplashScreen():
    ...
    try:
        ...
        tk.Label(splash, text="QR Kod Uygulaması", font=("Helvetica", 18, "bold"), bg="#ffffff").pack(pady=30)

    tk.Label(splash, text="QR sistemi başlatılıyor...", font=("Arial", 11), bg="#ffffff", fg="#555555").pack(pady=10)
...
try:
    ...
    pass
window.title("QR Kod Uygulaması")
window.state('zoomed') 
...
def saveToFirebase(data):
    doc_ref = db.collection("qrCode").document(data["Sıra No"])
    doc_ref.set(data)
...
def createQr():
    ...
    try:
        if dateVar.get() and dateEntry.get().strip():
            # Tarihi doğrula
            manuelDate = datetime.strptime(dateEntry.get(), "%d.%m.%Y")
            dateStr = manuelDate.strftime("%d.%m.%Y ")
        else:
            dateStr = datetime.now().strftime("%d.%m.%Y %H:%M:%S")
    except ValueError:
        messagebox.showerror("Tarih Hatası", "Lütfen tarihi şu formatta girin:\nGG.AA.YYYY\nÖrn: 17.04.2025")
        return

    qrPath = os.path.join(qrFolder, f"QR_{ queueNumber}.png")
    qrcode.make("\n".join([f"{k}: {v}" for k, v in data.items()])).save(qrPath)

    lastQrPath = qrPath
    img_open = Image.open(qrPath).resize((200, 200))
    img_tk = ImageTk.PhotoImage(img_open)
    qrLabel.config(image=img_tk)
    qrLabel.image = img_tk

    saveToCsv(data)
    saveToFirebase(data)
    incrementQueueNumber( queueNumber + 1)
    tree.insert("", tk.END, values=[data[col] for col in columns])
    messagebox.showinfo("Başarılı", f"{ queueNumber} numaralı QR oluşturuldu.")

def qrPrint():
    if not lastQrPath:
        messagebox.showwarning("Uyarı", "Henüz bir QR oluşturulmadı.")
        return
    try:
        if platform.system() == "Windows":
            os.startfile(lastQrPath, "print")
        else:
            subprocess.run(["lp", lastQrPath])
        messagebox.showinfo("Yazdırılıyor", "QR yazıcıya gönderildi.")
    except Exception as e:
        messagebox.showerror("Hata", f"Yazdırma hatası: {str(e)}")

def printQrSelectedRow():
    selected = tree.focus()
    if not selected:
    ...
    if not os.path.exists(qrPath):
        messagebox.showerror("Hata", "QR görseli bulunamadı.")
        return
    try:
        if platform.system() == "Windows":
            os.startfile(qrPath, "print")
        else:
            subprocess.run(["lp", qrPath])
        messagebox.showinfo("Yazdırılıyor", f"{ queueNumber} numaralı QR yazdırılıyor.")
    except Exception as e:
...
def createQrFromFile(data):
    ...
    incrementQueueNumber( queueNumber + 1)
    tree.insert("", tk.END, values=[data[col] for col in columns])
    messagebox.showinfo("Yeni QR", f"{ queueNumber} numaralı yeni QR oluşturuldu.")
...
def orderModel():
    ...
    btn_qr_frame = tk.Frame(window, bg="#f5f5f5")
    btn_qr_frame.pack(pady=5)

    tk.Button(
        btn_qr_frame,
        text="Kaydet ve QR Oluştur",
        command=saveAndNewQr,
        bg="#1e88e5", fg="white"
    ).pack(side="left")
    ...
    tk.Button(
        btn_qr_frame,
        image=help_tk,
        command=lambda: messagebox.showinfo("Yardım", "Bu buton güncellenmiş veriyle yeni bir QR oluşturur."),
        bg="#f5f5f5",
        activebackground="#f5f5f5",
        highlightthickness=0,
        bd=0,
...
qr_frame = tk.Frame(tab)
try:
    bg_img = ImageTk.PhotoImage(Image.open(resource_path("background.png")).resize((1000, 700)))
    bg_label = tk.Label(qr_frame, image=bg_img)
    bg_label.place(x=0, y=0, relwidth=1, relheight=1)
    qr_frame.bg_img = bg_img
except:
    qr_frame.configure(bg="#eef5fc")
...
tab.add(qr_frame, text="QR Oluştur")
...
entry_gonderen = nemInput(qr_frame, "Teslim Eden:")
entry_ariza = nemInput(qr_frame, "Arıza:")
entry_teslim = nemInput(qr_frame, "Teslim Alan:")
entry_personel = nemInput(qr_frame, "İlgili Personel:")
entry_tip = nemInput(qr_frame, "Cihaz Tipi:")
entry_model = nemInput(qr_frame, "Marka/Model:")
entry_durum = nemInput(qr_frame, "Durum:")
entry_aciklama = nemInput(qr_frame, "Açıklama:")

# Tarih seçme kutusu ve giriş alanı
dateVar = tk.BooleanVar()
dateCheckbox = tk.Checkbutton(qr_frame, text="Tarihi kendim seçmek istiyorum", variable=dateVar, command=lambda: dateEntry.config(state="normal" if dateVar.get() else "disabled"))
dateCheckbox.pack()

dateEntry = tk.Entry(qr_frame, width=40, state="disabled")
dateEntry.insert(0, "GG.AA.YYYY")
dateEntry.pack(pady=5)


tk.Button(qr_frame, text="QR Oluştur", command=createQr).pack(pady=10)
tk.Button(qr_frame, text="Son QR'ı Yazdır", command=qrPrint).pack(pady=5)
qrLabel = tk.Label(qr_frame)
qrLabel.pack(pady=10)
...
tk.Button(
    btnOrderFrame,
    image=help_tk,
    command=lambda: messagebox.showinfo("Yardım", "Listeden seçilen kaydı düzenlemenizi veya yeni QR oluşturmanızı sağlar."),
    bg="#eef5fc",
...
tk.Button(
    btnPrintFrame,
    image=help_tk,
    command=lambda: messagebox.showinfo("Yardım", "Listede seçilen satıra ait QR görselini yazıcıya gönderir."),
    bg="#eef5fc",
...
tree.bind("<Double-1>", lambda e: printQrSelectedRow())
